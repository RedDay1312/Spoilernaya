  ...
  return (
    <div className="absolute inset-x-0 bottom-0 z-30 p-3 sm:p-5">
      ...
      <div className="mx-auto flex w-full max-w-3xl items-end gap-3">
        {portrait ? (
          <div className="hidden h-28 w-28 shrink-0 overflow-hidden rounded-xl border border-line bg-surface sm:block">
        ...
        <div className="min-w-0 flex-1 rounded-xl border border-line bg-surface/95 p-4 shadow-[0_18px_40px_rgba(0,0,0,0.45)]">
          ...
          {choices.length > 0 ? (
            <div className="mt-4 flex flex-col gap-2">
              {choices.map((c) => (
                <button
                  ...
                  type="button"
                  onClick={() => pick(c)}
                  className="rounded-md border border-line bg-surface-2 px-3 py-2.5 text-left text-sm leading-snug text-fg transition-colors hover:bg-bg"
                >
                  {c.text}
                </button>
              ))}
            ...
            <p className="mt-3 text-xs text-subtle">Нажми, чтобы продолжить</p>
          )}
        </div>
      </div>
    </div>
